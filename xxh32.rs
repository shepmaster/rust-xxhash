...
unsafe fn xxh32_impl(input: &[u8], seed: u32) -> u32 { #![inline(always)]
    let data: *const u8 = input.repr().data;
    let mut p: *const u32 = transmute(data);
    let len = input.len();
    ...
    let rem = len & 3;
    let mut bp: *const u8 = transmute(p);
    for _ in range(0, rem) {
...
impl XXState {
    ...
    unsafe fn update_impl(&mut self, input: &[u8]) { #![inline(always)]
        let mem: *mut u8 = &self.memory as *const _ as *mut u8;
        ...
        if self.memsize + (len as int) < 16 {
            ...
            let dst: *mut u8 = mem.offset(self.memsize);
        ...
        if self.memsize != 0 {
            ...
            let mut p: *const u32 = transmute(&self.memory);
        ...
        if rem > 0 {
            intrinsics::copy_nonoverlapping_memory(mem, data, rem as uint);
...
mod c {
    ...
    fn test() {
        ...
        let test = |len: c_int, seed: u32, expected: u32| {
            let result = unsafe { XXH32(buf as *const c_void, len as c_int, seed as c_uint) } as u32;
        ...
        let test = |size: c_int, seed: u32, expected: u32| {
            unsafe {
                ...
                for chunk in range(0, chunks) {
                    XXH32_update(state, buf.offset(chunk as int *15) as *const c_void, 15 as c_int, XXH_LittleEndian);
                }
                XXH32_update(state, buf.offset(chunks as int * 15) as *const c_void, rem, XXH_LittleEndian);
    ...
    fn oneshot(bench: &mut Bencher) {
        ...
        let buf: *mut u8 = unsafe { libc::malloc(BUFSIZE as size_t) as *mut u8 };
        for i in range(0, BUFSIZE as int) {
           // v.push((i & 0xFF) as u8);
           unsafe { *buf.offset(i) = (i & 0xFF) as u8; }
        }

        bench.iter(|| unsafe { XXH32(buf as *const c_void, BUFSIZE, 0); });
    ...
    fn bench_chunks_base(bench: &mut Bencher, chunk_size: i32) {
        ...
        bench.iter(|| unsafe {
            ...
            for chunk in range(0, chunks) {
                XXH32_update(state, buf.offset(chunk as int * chunk_size as int) as *const c_void, chunk_size as c_int, XXH_LittleEndian);
            }
            XXH32_update(state, buf.offset(chunks as int * chunk_size as int) as *const c_void, rem, XXH_LittleEndian);
            XXH32_digest(state);
        });
